...
for file in files:
    flag_front_back=file.split('_')[-2]
    if flag_front_back=='1':
        index=np.random.randint(0,4)
        back_img_name=file.split('_')[0]+'_'+'0'+'_'+str(index)+'.jpg'

        front=cv2.imread('/home/simple/mydemo/ocr_project/idcard_generator_project/gen_data_with_logo/'+file)
        h,w,c=front.shape
        front=front[:,0:int(w/2),:]
        back=cv2.imread('/home/simple/mydemo/ocr_project/idcard_generator_project/gen_data_with_logo/'+back_img_name)
        h_back,w_back,c_back=back.shape
        back=back[:,0:int(w_back/2),:]
