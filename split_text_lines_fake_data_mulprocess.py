...
def gen_faker_card_run(images_name_index):
    index,images_d=images_name_index
...
if __name__ == "__main__":
    ...
    for index,image_name in enumerate(images_name):
        images_name_index.append((index,image_name))
    pool=Pool(10)
    pool.map(gen_faker_card_run,images_name_index)
    # pool.close()
    # pool.join()
    print('done')
